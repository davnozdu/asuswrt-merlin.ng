...
export GT-BE98 += BUILD_NAME="GT-BE98" EXT_PHY="BCM84880" NVSIZE="192" DHDAP=y DPSTA=n REPEATER=y IPV6SUPP=y HTTPS=y ARM=y \
		...
		OPTIMIZE_XBOX=y ODMPID=y BCMSMP=y XHCI=y DUALWAN=y NEW_USER_LOW_RSSI=y OPENVPN=y TIMEMACHINE=y MDNS=y VPNC=y \
		JFFS2LOG=y BWDPI=y DUMP_OOPS_MSG=n LINUX_MTD="64" DEBUGFS=y TEMPROOTFS=n SSH=y EMAIL=y FRS_FEEDBACK=n SYSSTATE=y \
		ROG=y STAINFO=y CLOUDCHECK=n NATNL_AICLOUD=n REBOOT_SCHEDULE=y MULTICASTIPTV=y QUAGGA=y WLCLMLOAD=n BCM_MUMIMO=y \
		LAN50="all" ATCOVER=y GETREALIP=y CFEZ=y ETLAN_LED=y TFAT=y NTFS="tuxera" HFS="tuxera" NEWSSID_REV4=y NEW_APP_ARM=y \
		NETOOL=y TRACEROUTE=y FORCE_AUTO_UPGRADE=n ALEXA=n SW_HW_AUTH=y HD_SPINDOWN=y ASPMD=n BCM_MEVENT=n BCMEVENTD=y \
		BCM_APPEVENTD=n LETSENCRYPT=y VPN_FUSION=y JFFS_NVRAM=y NVRAM_ENCRYPT=y NATNL_AIHOME=n BCM_CEVENTD=y UTF8_SSID=y \
		I2CTOOLS=y AMAS=y DBLOG=y ETHOBD=y CFGSYNC_LOCSYNC=y CONNDIAG=y IPSEC=STRONGSWAN IPSEC_SRVCLI_ONLY=SRV DWB=y OPEN_NAT=y \
		ROG_UI=n AMASDB=n CRASHLOG=y WATCH_REINIT=n BW160M=y BRCM_HOSTAPD=y UUPLUGIN=y IPERF3=y INFO_EXAP=y BCN_RPT=y \
		BTM_11V=y INTERNETCTRL=y ASD=n AHS=n FRS_LIVE_UPDATE=n ASUSCTRL=y HAS_5G_2=y HAS_6G_2=n CAPTCHA=y OUTFOX=y \
		GAME_MODE=y AMAS_ETHDETECT=y INSTANT_GUARD=y AVBLCHAN=y FRONTHAUL_DWB=y BCM_CLED=y BONDING=y BONDING_WAN=n \
		ACL96=y LACP=y OWE_TRANS=y DHD_6715B0=y EXTEND_LIMIT=y GOOGLE_ASST=n QUADBAND=y IPV6S46=y OCNVC=y DSLITE=y \
		BTN_TURBO=y WIREGUARD=y BCMBSD_V2=y SW_CTRL_ALLLED=y COMFW=y NFCM=n DNSQUERY_INTERCEPT=y WIFI7=y BW320M=y \
		MTLANCFG=y CAPTIVE_PORTAL=y CP_FREEWIFI=y WPA3_ENTERPRISE=y \
		EXTRA_KERNEL_CONFIGS="BCM_JUMBO_FRAME=y BCM_MAX_MTU_SIZE=10240 CONFIG_BCM_IGNORE_BRIDGE_MTU=y" MAX_VPNF=4 \
		WTFAST_V2=y AMAS_OB_DISABLE_DSAP=y USB_WAN_BACKUP=y AUTO_WANPORT=y \
		NFS=y SNMPD=y TOR=n UPNPIGD2=y NANO=y NTPD=y DNSPRIVACY=y IPSET=y VPN_FUSION_MERLIN=y
		NFS=n SNMPD=y TOR=n UPNPIGD2=y NANO=y NTPD=y IPSET=y VPN_FUSION_MERLIN=y

# De-clouded: no Alexa/Google Assistant, no AAE/mastiff tunnel (ASUS Router app
# remote access), no asd phone-home, no ASUS account binding / AWS IoT.  libasc
# stays because the prebuilt networkmap/arpstorm/find_cap/asusdiscovery and rc
# link against it; the config generator only kept it as a side effect of ASD.
export GT-BE98 += LIBASC=y ACCOUNT_BINDING=n AWSIOT=n
