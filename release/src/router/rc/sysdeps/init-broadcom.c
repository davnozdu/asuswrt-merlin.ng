...
void hnd_nat_ac_init(int bootup)
{
	...
	// A.QOS/BCMTM/HW QoS Classful : not to disable fc
	nvram_set_int("fc_disable", nvram_get_int("fc_disable_force") || (routing_mode && IS_NON_AQOS() && !IS_RB_QOS() && !IS_BCMTM_QOS() && !IS_HWQOS_CLASSFUL()) ? 1 : 0);

	// A.QOS/BCMTM : no need to disable runner
	nvram_set_int("runner_disable", nvram_get_int("runner_disable_force") || (routing_mode && IS_NON_AQOS() && !IS_RB_QOS() && !IS_BCMTM_QOS() && !IS_HWQOS_CLASSFUL()) ? 1 : 0);
